...
Status: implementation specification, not an enabled account system. Prepared for the v0.27.25 draft on 20 September 2026.
...
3. Create an authoritative collection membership store. Server checks must derive identity and role from the verified session, never browser-supplied role fields. A temporary grant needs an explicit expiry or revocation condition and a server-enforced revoked state.
...
- Email for the permanent Owner.
- Email for the Temporary Owner; confirm if this is the Administrator's identity.
...
Passwords are to be entered by their owners through the secure setup flow, not sent in chat. No accounts, passwords, invitations, new paid services or Cloudflare configuration were created by this draft.
