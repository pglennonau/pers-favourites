-- Pers Favourites v026 - one isolated Supabase project per rollout.
-- v026 adds moderated venue photos. Ordinary browsing is public/read-only; protected access is required only for Owner/Admin controls or photo contribution.
...
create table if not exists public.places (
  ...
  address text, lat double precision, lng double precision, price text,
...
revoke all on table public.profiles, public.app_settings, public.places, public.venue_photos, public.personal_place_data, public.visits, public.user_preferences, public.audit_snapshots from anon, authenticated;
