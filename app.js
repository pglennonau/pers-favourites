...
function renderExternalResults(){
  ...
  const groups=[
    {id:'externalGoogleResultsPanel',title:t('moreGoogle'),rows:rows.filter(p=>p.provider==='Google Places')},
    {id:'externalTripadvisorResultsPanel',title:t('tripadvisorResults'),rows:rows.filter(p=>p.provider==='TripAdvisor')}
  ];
  let anchor=$('listPanel');
  const card=p=>{const d=distanceFor(p),rating=p.provider==='TripAdvisor'?p.tripadvisorRating:p.googleRating,count=p.provider==='TripAdvisor'?p.tripadvisorRatingCount:p.googleRatingCount,url=p.provider==='TripAdvisor'?p.tripadvisorUrl:p.googleMapsUrl;return `<article class="place-card external-place-card"><div class="place-card-inner"><div class="place-card-top"><div><h3>${esc(p.name)}</h3><div class="meta">${esc([p.placeType,p.cuisine,p.city,p.price,d!=null?(d<1?Math.round(d*1000)+' m':d.toFixed(1)+' km'):''].filter(Boolean).join(' · '))}</div><span class="source-pill">${esc(p.provider)}</span></div><div class="rating-pair">${rating?`<span class="rating-pill">${p.provider==='TripAdvisor'&&safeUrl(p.tripadvisorRatingIconUrl)?`<img class="tripadvisor-rating-icon" src="${esc(safeUrl(p.tripadvisorRatingIconUrl))}" alt="TripAdvisor rating" /> `:''}${esc(p.provider)} ${p.provider==='TripAdvisor'&&safeUrl(p.tripadvisorRatingIconUrl)?'':'★ '}${(+rating).toFixed(1)}${count?` (${count})`:''}</span>`:''}${p.openNow===true?`<span class="status-pill">${esc(t('openNow'))}</span>`:''}</div></div><div class="card-actions">${safeUrl(url)?`<a href="${esc(safeUrl(url))}" target="_blank" rel="noopener">${p.provider==='TripAdvisor'?'View on TripAdvisor':'Open Google'}</a>`:''}${safeUrl(p.website)?`<a href="${esc(safeUrl(p.website))}" target="_blank" rel="noopener">Website</a>`:''}${canEdit()?`<button data-add-external="${esc(externalKey(p))}">Add to Pers</button>`:''}</div></div></article>`;};
  for(const g of groups){
    if(!g.rows.length)continue;
    const panel=document.createElement('section');panel.id=g.id;panel.className='external-results-panel';
    panel.innerHTML=`<div class="results-head"><div><strong>${esc(g.title)}</strong><span class="result-count">${g.rows.length}</span></div></div>`+g.rows.map(card).join('');
...
function googleAttributionHtml(photo={}){
  ...
  return `<span class="google-maps-attribution" translate="no">Google Maps</span>${authors?` · Photo by ${authors}`:''}${source?` · <a href="${esc(source)}" target="_blank" rel="noopener">View source photo</a>`:''}${report?` · <a href="${esc(report)}" target="_blank" rel="noopener">Report</a>`:''}`;
}
