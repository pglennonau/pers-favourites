...
const CFG = Object.assign({version:'0.27.15',mode:'local',appName:'Pers Favourites',ownerDisplayName:'Owner',homeRegion:'',allowViewerSignup:false,supabasePublishableKey:'',supabaseAnonKey:'',placesSearchEndpoint:''}, window.PERS_CONFIG || {});
...
function defaultLocalState(){
  ...
  return {
    ...
    settings:{appName:CFG.appName,ownerDisplayName:CFG.ownerDisplayName,homeRegion:CFG.homeRegion,allowUserPhotos:true,defaultNearMe:'1',quickCoffee:true,askPersEnabled:false,askPersEndpoint:'',placesSearchEndpoint:clean(CFG.placesSearchEndpoint)},
    users:[
...
function isSystemAdmin(){return currentUser?.role==='sysadmin';}
function canEdit(){ return ['owner','sysadmin','admin'].includes(currentUser?.role); }
...
function applyPreferencesToUI(){$('sortSelect').value=preferences.sort||'nearest';$('prefDistance').value=preferences.preferredDistance||'1';if($('settingDefaultNearMe'))$('settingDefaultNearMe').value=state.settings.defaultNearMe||'1';if($('settingQuickCoffee'))$('settingQuickCoffee').checked=state.settings.quickCoffee!==false;if($('roleAreaTitle'))$('roleAreaTitle').textContent=isSystemAdmin()?'System Administrator':'Owner';const qb=document.querySelector('[data-quick="coffee"]');if(qb)qb.classList.toggle('hidden',state.settings.quickCoffee===false);$('rememberFilters').checked=!!preferences.rememberFilters;setView(preferences.view||'list',false);syncFilterControls();}
...
function renderPhotoGallery(placeId){
  ...
  const tiles=xs.map(ph=>{
    ...
    const manage=canEdit()?`<div class="photo-actions"><button data-photo-caption="${ph.id}">Caption</button>${ph.status!=='approved'?`<button data-photo-approve="${ph.id}">Approve</button>`:''}${ph.status!=='hidden'?`<button data-photo-hide="${ph.id}">Hide</button>`:''}${ph.status==='approved'&&!ph.isCover?`<button data-photo-cover="${ph.id}">Add to banner</button>`:''}${ph.status==='approved'&&!ph.isCover?`<button data-photo-earlier="${ph.id}">Move earlier</button><button data-photo-later="${ph.id}">Move later</button>`:''}<button data-photo-delete="${ph.id}">Delete</button></div>`:(ph.uploadedBy===currentUser?.id?`<div class="photo-actions"><button data-photo-delete="${ph.id}">Remove my photo</button></div>`:'');
...
function openDetail(id){
  ...
  const d=distanceFor(p),banner=bannerPhotosForPlace(id),ownerName=state?.settings?.ownerDisplayName||CFG.ownerDisplayName||'Owner';const hero=banner.length?`<div class="detail-hero banner-${banner.length}">${banner.map(ph=>`<img data-photo-id="${ph.id}" alt="Photo of ${esc(p.name)}" />`).join('')}</div>`:`<div class="detail-hero"><div class="detail-hero-empty">${esc(p.name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'PF')}</div></div>`;
...
async function quickFilter(which){if(which==='near'||which==='coffee'){toast('Updating location…');currentPosition=null;const loc=await getDeviceLocation();if(!loc){toast(lastGeoError||'Fresh location unavailable. Retry Near Me.');return;}filters.distance=preferences.preferredDistance||state?.settings?.defaultNearMe||'1';if(which==='coffee'){filters.type='Cafe - Specialty Coffee';filters.greatFor='Coffee';}}if(which==='five')filters.rating=filters.rating==='5'?'':'5';if(which==='favourite')filters.status=filters.status==='favourite'?'':'favourite';if(which==='want')filters.status=filters.status==='want'?'':'want';if(which==='restaurant')filters.type=filters.type==='Restaurant'?'':'Restaurant';if(which==='winebar')filters.type=filters.type==='Wine Bar'?'':'Wine Bar';populateFilterOptions();render();}
function openAuth(purpose='owner'){
  authPurpose=purpose;
  $('authTitle').textContent=purpose==='contributor'?'Contribute venue photos':purpose==='rating'?'Rate this venue':'Owner / System Administrator sign in';
  $('authIntro').textContent=purpose==='contributor'?'Browsing never requires a login. To contribute a photo, enter your email and use the secure sign-in link we send you.':purpose==='rating'?'Browsing never requires a login. To add a public User Rating, identify yourself with the secure email sign-in link. Owner/Admin ratings are excluded from the User Rating average.':'Owner/System Administrator controls are protected. Sign in with the rollout account password. On supported phones, your password manager can use Face ID/biometrics to fill it.';
