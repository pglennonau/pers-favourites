...
const CFG = Object.assign({version:'0.27.7',mode:'local',appName:'Pers Favourites',ownerDisplayName:'Owner',homeRegion:'',allowViewerSignup:false,supabasePublishableKey:'',supabaseAnonKey:'',placesSearchEndpoint:''}, window.PERS_CONFIG || {});
...
async function exportBackup(){
  ...
  exportJson(payload,`pers-favourites-v027g-full-backup-${new Date().toISOString().slice(0,10)}.json`);
...
async function exportHistory(){if(CFG.mode==='local'){exportJson({appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),history:state.history},`pers-favourites-v027g-history-${new Date().toISOString().slice(0,10)}.json`);return;}try{const rows=await rest('audit_snapshots',{filters:'&order=created_at.desc&limit=50'});exportJson({appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),history:rows},`pers-favourites-v027g-history-${new Date().toISOString().slice(0,10)}.json`);}catch(e){toast(e.message);}}
...
function openAccount(){
  ...
  $('settingAppName').value=state.settings.appName||'';$('settingOwnerName').value=state.settings.ownerDisplayName||'';$('settingHomeRegion').value=state.settings.homeRegion||'';$('settingAllowUserPhotos').checked=state.settings.allowUserPhotos!==false;$('settingAskPersEnabled').checked=!!state.settings.askPersEnabled;$('settingAskPersEndpoint').value=state.settings.askPersEndpoint||'';$('settingPlacesSearchEndpoint').value=state.settings.placesSearchEndpoint||CFG.placesSearchEndpoint||'';$('placesEndpointStatus').textContent=placesEndpoint()?'Configured - tap Test Google Places connection to verify.':'Not connected - current in-app search will use OpenStreetMap fallback.';$('rememberFilters').checked=!!preferences.rememberFilters;$('prefDistance').value=preferences.preferredDistance||'5';updatePendingPhotoBadge();$('signOutBtn').classList.toggle('hidden',CFG.mode!=='local'&&!session);$('accountDialog').showModal();
}
