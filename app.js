...
const CFG = Object.assign({version:'0.27.6',mode:'local',appName:'Pers Favourites',ownerDisplayName:'Owner',homeRegion:'',allowViewerSignup:false,supabasePublishableKey:'',supabaseAnonKey:'',placesSearchEndpoint:''}, window.PERS_CONFIG || {});
...
async function overpassSearch(q,loc,radiusKm=80){if(!loc||foldText(q).length<2)return [];const needle=regexEscape(q).replace(/\s+/g,'\\s+');const radius=Math.round(radiusKm*1000);const query=`[out:json][timeout:12];(nwr(around:${radius},${loc.lat},${loc.lng})[\"name\"~\"${needle}\",i];nwr(around:${radius},${loc.lat},${loc.lng})[\"brand\"~\"${needle}\",i];);out center tags 40;`;const u=new URL('https://overpass-api.de/api/interpreter');u.searchParams.set('data',query);const r=await fetch(u.toString(),{headers:{'Accept':'application/json'}});if(!r.ok)return [];const j=await r.json();return Array.isArray(j?.elements)?j.elements:[];}
...
async function runFindPlaceOnline(widen=false){
  ...
  try{
    const loc=await getDeviceLocation();const ctx=loc?await reverseLocationContext(loc):storedLocationContext();lastFindPlaceContext=ctx;const q=normalizeOnlineQuery(raw);let rows=[];const configured=!!clean(state?.settings?.placesSearchEndpoint||CFG.placesSearchEndpoint);
    if(configured){const gr=await googlePlacesSearch(raw,loc,ctx,widen);rows.push(...(gr||[]).map(x=>({place:googleWorkerResultToPlace(x),provider:'Google Places'})));}
    if(!widen){
      ...
      else{$('findPlaceStatus').textContent=`${lastGeoError||'Current location is unavailable.'} Choose Country/State/City in the filters or enable location, then retry. No worldwide search was performed.`;$('searchWiderBtn').classList.remove('hidden');return;}
    ...
    if(onlinePlaceResults.length){$('findPlaceStatus').textContent=`${widen?'Showing wider matches':'Showing nearby matches'}${area}${loc&&currentPosition?.accuracy?` (location accuracy about ${Math.round(currentPosition.accuracy)} m)`:''}.`;if(!widen)$('searchWiderBtn').classList.remove('hidden');renderOnlineResults();return;}
    const locationNote=loc?'':` ${lastGeoError||'Location was unavailable.'}`;$('findPlaceStatus').innerHTML=`No ${widen?'wider':'nearby'} match found${esc(area)}.${esc(locationNote)} ${!widen?'No worldwide results were substituted. ':''}<a href="${esc(googleMapsSearchUrl(raw,ctx))}" target="_blank" rel="noopener">Check Google Maps</a>.`;if(!widen)$('searchWiderBtn').classList.remove('hidden');
...
async function exportBackup(){
  ...
  exportJson(payload,`pers-favourites-v027f-full-backup-${new Date().toISOString().slice(0,10)}.json`);
...
async function exportHistory(){if(CFG.mode==='local'){exportJson({appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),history:state.history},`pers-favourites-v027f-history-${new Date().toISOString().slice(0,10)}.json`);return;}try{const rows=await rest('audit_snapshots',{filters:'&order=created_at.desc&limit=50'});exportJson({appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),history:rows},`pers-favourites-v027f-history-${new Date().toISOString().slice(0,10)}.json`);}catch(e){toast(e.message);}}
