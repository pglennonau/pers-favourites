...
function populateFilterOptions(){
  ...
  for(let pass=0;pass<2;pass++){
    ...
    for(const [id,label,key] of dynamic){const rows=rowsForFilterChoice(key),counts=new Map(valuesForFilter(rows,key).filter(Boolean).map(v=>[v,0]));for(const v of valuesForFilter(rows,key).filter(Boolean))counts.set(v,(counts.get(v)||0)+1);const options=uniq([...masterValues(key),...counts.keys()]).map(value=>({value,label:counts.get(value)?`${value} (${counts.get(value)})`:value}));if(filters[key]&&!optionContains(options,filters[key],false))filters[key]='';populateSelect(id,label,options,filters[key]);}
  }
  syncFilterControls();
}
