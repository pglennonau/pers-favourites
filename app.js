...
const CFG = Object.assign({version:'0.27.16',mode:'local',appName:'Pers Favourites',ownerDisplayName:'Owner',homeRegion:'',allowViewerSignup:false,supabasePublishableKey:'',supabaseAnonKey:'',placesSearchEndpoint:''}, window.PERS_CONFIG || {});
...
function syncLocationBanner(){const scope=[filters.city,filters.region,filters.country].filter(Boolean).join(', ');if($('locationScopeLabel'))$('locationScopeLabel').textContent=scope||'All locations';if($('clearLocationBtn'))$('clearLocationBtn').disabled=!scope;}
function syncQuickActions(){const active={near:!!filters.distance,coffee:filters.type==='Cafe - Specialty Coffee'&&filters.greatFor==='Coffee',five:filters.rating==='combined5',favourite:filters.status==='favourite',want:filters.status==='want',restaurant:filters.type==='Restaurant',winebar:filters.type==='Wine Bar'};document.querySelectorAll('#quickActions [data-quick]').forEach(b=>b.classList.toggle('active',!!active[b.dataset.quick]));}
function render(){applyFilters();renderChips();renderList();if(preferences.view==='map')renderMap();$('resultCount').textContent=`${filteredPlaces.length} ${filteredPlaces.length===1?'place':'places'}${archiveMode?' in Archive':''}`;$('adminActions').classList.toggle('hidden',!canEdit()||archiveMode);syncLocationBanner();syncQuickActions();updatePendingPhotoBadge();persistPreferences();}
...
function bindEvents(){
  ...
  $('clearLocationBtn').onclick=()=>{filters.country='';filters.region='';filters.city='';rebuildAndRender();};
