...
const CFG = Object.assign({version:'0.27.5',mode:'local',appName:'Pers Favourites',ownerDisplayName:'Owner',homeRegion:'',allowViewerSignup:false,supabasePublishableKey:'',supabaseAnonKey:''}, window.PERS_CONFIG || {});
...
const LS_DB = `pers-v027e-db:${CFG.deploymentId || location.pathname}`;
const LS_SESSION = `pers-v027e-session:${CFG.deploymentId || location.pathname}`;
const LS_PUBLIC_VIEWER = `pers-v027e-public-viewer:${CFG.deploymentId || location.pathname}`;
...
function defaultLocalState(){
  ...
  return {
    schema:275, appVersion:CFG.version, createdAt:nowISO(), updatedAt:nowISO(),
    settings:{appName:CFG.appName,ownerDisplayName:CFG.ownerDisplayName,homeRegion:CFG.homeRegion,allowUserPhotos:true,askPersEnabled:false,askPersEndpoint:''},
    users:[
...
function findLegacyLocalState(){
  const candidates=[`pers-v026-db:${CFG.deploymentId||location.pathname}`,'persLocalTrialDbV1','persLocalTrialDbV2',`pers-v025-db:${CFG.deploymentId||location.pathname}`,`pers-v024-db:${CFG.deploymentId||location.pathname}`,`pers-v023-db:${CFG.deploymentId||location.pathname}`];
...
function normalizeOnlineQuery(q){return clean(q).replace(/\b(restaurant|restaurante|cafe|café|bar|bistro|winery|wine bar|hotel|pub)\b/ig,' ').replace(/\s+/g,' ').trim()||clean(q);}
...
function getDeviceLocation(){return new Promise((resolve)=>{if(currentPosition)return resolve(currentPosition);if(!navigator.geolocation)return resolve(null);navigator.geolocation.getCurrentPosition(pos=>{currentPosition={lat:pos.coords.latitude,lng:pos.coords.longitude};resolve(currentPosition);},()=>resolve(null),{enableHighAccuracy:true,timeout:7000,maximumAge:300000});});}
...
async function nominatimSearch(q,loc,bounded){const u=new URL('https://nominatim.openstreetmap.org/search');u.searchParams.set('format','jsonv2');u.searchParams.set('addressdetails','1');u.searchParams.set('namedetails','1');u.searchParams.set('extratags','1');u.searchParams.set('limit','10');u.searchParams.set('q',q);if(loc){const dlat=.35,dlon=.45;u.searchParams.set('viewbox',`${loc.lng-dlon},${loc.lat+dlat},${loc.lng+dlon},${loc.lat-dlat}`);if(bounded)u.searchParams.set('bounded','1');}const r=await fetch(u.toString(),{headers:{'Accept':'application/json'}});if(!r.ok)throw new Error(`Online place search failed (${r.status}).`);return r.json();}
function onlineResultToPlace(r){const a=r.address||{},e=r.extratags||{},name=clean(r.namedetails?.name||r.name||String(r.display_name||'').split(',')[0]);const lat=+r.lat,lng=+r.lon;return {id:uid(),name,placeType:clean(r.type||r.category),cuisine:clean(e.cuisine),country:clean(a.country),stateRegion:clean(a.state||a.region||a.county),city:clean(a.city||a.town||a.village||a.municipality),suburb:clean(a.suburb||a.neighbourhood||a.quarter),address:clean(r.display_name),lat,lng,price:'',persRating:0,phone:clean(e.phone||e['contact:phone']),website:clean(e.website||e['contact:website']),googleMapsUrl:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${lat},${lng}`)}`,bookingUrl:'',mealTypes:[],greatFor:[],features:[],dietary:[],tags:['Find Place Online'],mustTry:'',notes:'',createdAt:nowISO(),updatedAt:nowISO(),archivedAt:''};}
async function runFindPlaceOnline(){const raw=clean($('findPlaceQuery').value||$('placeName').value);if(!raw)return toast('Enter a venue name first.');$('findPlaceStatus').textContent='Searching nearby first…';$('findPlaceResults').innerHTML='';try{const loc=await getDeviceLocation();const ctx=loc?await reverseLocationContext(loc):storedLocationContext();const q=normalizeOnlineQuery(raw);let rows=[];if(loc)rows=await nominatimSearch(q,loc,true);if(rows.length<3&&ctx.label){const localQ=`${q}, ${ctx.label}`;const contextual=await nominatimSearch(localQ,loc,false);const ids=new Set(rows.map(x=>`${x.osm_type}:${x.osm_id}`));for(const x of contextual)if(!ids.has(`${x.osm_type}:${x.osm_id}`)){rows.push(x);ids.add(`${x.osm_type}:${x.osm_id}`);}}let widened=false;if(rows.length<3){widened=true;const extra=await nominatimSearch(q,loc,false);const ids=new Set(rows.map(x=>`${x.osm_type}:${x.osm_id}`));for(const x of extra)if(!ids.has(`${x.osm_type}:${x.osm_id}`))rows.push(x);}
  onlinePlaceResults=rows.map(r=>({raw:r,place:onlineResultToPlace(r),distance:loc?distanceKm(loc,{lat:+r.lat,lng:+r.lon}):null})).sort((a,b)=>(a.distance??99999)-(b.distance??99999)||(+b.raw.importance||0)-(+a.raw.importance||0));
  const area=ctx.label?` around ${ctx.label}`:'';$('findPlaceStatus').textContent=onlinePlaceResults.length?(widened?`Nearby matches${area} are prioritised; wider results are shown only as fallback.`:`Showing the best nearby matches${area}.`):'No matches found.';
  $('findPlaceResults').innerHTML=onlinePlaceResults.map((x,i)=>`<div class="online-result"><div class="online-result-top"><div><strong>${esc(x.place.name||'Unnamed place')}</strong><div class="small muted">${esc(x.place.address)}</div><div class="small">${x.distance==null?'':`${x.distance<1?Math.round(x.distance*1000)+' m':x.distance.toFixed(1)+' km'} away`}${x.place.website?` · Website found`:''}</div></div><button class="primary compact" data-use-online="${i}">Use</button></div></div>`).join('');
 }catch(e){$('findPlaceStatus').textContent=e.message||'Online search failed.';}}
...
function showApp(){
  ...
  if(CFG.mode!=='local'&&session){const pending=sessionStorage.getItem('pers-v027e-pending-photo-place');if(pending&&places.some(p=>p.id===pending)){sessionStorage.removeItem('pers-v027e-pending-photo-place');setTimeout(()=>openDetail(pending),150);}const rawRating=sessionStorage.getItem('pers-v027e-pending-rating');if(rawRating){sessionStorage.removeItem('pers-v027e-pending-rating');try{const pr=JSON.parse(rawRating);setTimeout(()=>setUserRating(pr.placeId,+pr.rating),180);}catch{}}}
}
...
async function setUserRating(placeId,n){
  ...
  if(CFG.mode!=='local'&&isAnonymousViewer()){pendingRating={placeId,rating:n};sessionStorage.setItem('pers-v027e-pending-rating',JSON.stringify(pendingRating));openAuth('rating');return;}
...
async function prepareSingleImport(){
  ...
  if(/^https?:\/\//i.test(text)){
    ...
    $('importDialog').close();openPlaceEditor();$('placeName').value=text;$('findPlaceQuery').value=text;$('findPlaceDialog').showModal();runFindPlaceOnline();
  }
}
...
async function exportBackup(){
  ...
  const payload={persFavouritesBackup:true,backupSchema:275,appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),database,settings:state.settings,places,photos,personal,visits,userRatings,ratingSummaries,preferences,photoFiles:[],warnings:[]};
  ...
  exportJson(payload,`pers-favourites-v027e-full-backup-${new Date().toISOString().slice(0,10)}.json`);
...
async function exportHistory(){if(CFG.mode==='local'){exportJson({appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),history:state.history},`pers-favourites-v027e-history-${new Date().toISOString().slice(0,10)}.json`);return;}try{const rows=await rest('audit_snapshots',{filters:'&order=created_at.desc&limit=50'});exportJson({appVersion:CFG.version,deploymentId:CFG.deploymentId,exportedAt:nowISO(),history:rows},`pers-favourites-v027e-history-${new Date().toISOString().slice(0,10)}.json`);}catch(e){toast(e.message);}}

function openAskPers(){if(!state?.settings?.askPersEnabled)return toast('Ask Pers is disabled by the Administrator.');$('askPersStatus').textContent='';$('askPersQuery').value='';$('askPersDialog').showModal();}
...
function openAccount(){
  ...
  $('settingAppName').value=state.settings.appName||'';$('settingOwnerName').value=state.settings.ownerDisplayName||'';$('settingHomeRegion').value=state.settings.homeRegion||'';$('settingAllowUserPhotos').checked=state.settings.allowUserPhotos!==false;$('settingAskPersEnabled').checked=!!state.settings.askPersEnabled;$('settingAskPersEndpoint').value=state.settings.askPersEndpoint||'';$('rememberFilters').checked=!!preferences.rememberFilters;$('prefDistance').value=preferences.preferredDistance||'5';updatePendingPhotoBadge();$('signOutBtn').classList.toggle('hidden',CFG.mode!=='local'&&!session);$('accountDialog').showModal();
}
async function saveCollectionSettings(){if(!isOwner())return toast('Only the Owner can change collection identity.');const s={...state.settings,appName:clean($('settingAppName').value)||CFG.appName,ownerDisplayName:clean($('settingOwnerName').value)||'Pers',homeRegion:clean($('settingHomeRegion').value),allowUserPhotos:!!$('settingAllowUserPhotos').checked,askPersEnabled:!!$('settingAskPersEnabled').checked,askPersEndpoint:clean($('settingAskPersEndpoint').value)};await makeCheckpoint('Before collection settings change');if(CFG.mode==='local'){state.settings=s;saveLocalState();}else{await rest('app_settings',{method:'PATCH',body:{app_name:s.appName,owner_display_name:s.ownerDisplayName,home_region:s.homeRegion,allow_user_photos:s.allowUserPhotos,ask_pers_enabled:s.askPersEnabled,ask_pers_endpoint:s.askPersEndpoint||null},filters:'?id=eq.1'});state.settings=s;}setBranding();toast('Collection settings saved');}
...
function bindEvents(){
  ...
  $('magicLinkBtn').onclick=async()=>{try{const email=clean($('loginEmail').value);if(!email)throw new Error('Enter your email address.');await sendMagicLink(email);if(authPurpose==='contributor')sessionStorage.setItem('pers-v027e-pending-photo-place',pendingContributionPlaceId||activeDetailPlaceId||'');if(authPurpose==='rating'&&pendingRating)sessionStorage.setItem('pers-v027e-pending-rating',JSON.stringify(pendingRating));$('authMessage').textContent='Secure sign-in link sent. Open the email on this device, then return here to add your photo.';}catch(e){$('authMessage').textContent=e.message;}};
